...
exports.addfeedback = async (req, res) => {
  try {
    const { student_id, tutor_id, course_id, rating, enjoy_most, review } =
      req.body;
    ...
    if (tutorCheck.rows.length === 0 || tutorCheck.rows[0].role !== "tutor") {
      return res.status(400).json({
        statusCode: 400,
    ...
    if (unlockCheck.rows.length === 0) {
      return res.status(400).json({
        statusCode: 400,
        message:
          "You can give feedback only after complete the course and assignment",
      });
    }

    const result = await con.query(
      `INSERT INTO tbl_feedback (student_id, tutor_id, course_id, rating, enjoy_most, review) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [
        student_id,
        tutor_id,
        course_id || null,
...
exports.deleteFeedback = async (req, res) => {
  try {
    ...
    const check = await con.query(
      `SELECT feedback_id FROM tbl_feedback WHERE feedback_id = $1`,
      [feedback_id],
    );
...
exports.updateFeedback = async (req, res) => {
  try {
    ...
    if (
      rating === undefined &&
      enjoy_most === undefined &&
      review === undefined
    ) {
      return res.status(400).json({
        statusCode: 400,
        message:
          "At least one field (rating, enjoy_most, review) is required to update",
      });
    }

    const result = await con.query(
      `UPDATE tbl_feedback SET rating = COALESCE($1, rating), enjoy_most = COALESCE($2, enjoy_most), review = COALESCE($3, review) WHERE feedback_id = $4 RETURNING *`,
...
exports.getTutorFeedbacks = async (req, res) => {
  try {
    ...
    const feedbackResult = await con.query(
      `SELECT f.feedback_id, f.rating, f.enjoy_most, f.review, f.response, f.feedback_created_at, u.user_id AS student_id, u.full_name AS student_name, c.course_id, c.course_title FROM tbl_feedback f JOIN tbl_user u ON f.student_id = u.user_id LEFT JOIN tbl_course c ON f.course_id = c.course_id WHERE f.tutor_id = $1 ORDER BY f.feedback_id DESC`,
      [tutor_id],
    );

    const ratingResult = await con.query(
      `SELECT COUNT(*) FILTER (WHERE rating = 5) AS total_5_rating, COUNT(*) FILTER (WHERE rating = 4) AS total_4_rating, COUNT(*) FILTER (WHERE rating = 3) AS total_3_rating, COUNT(*) FILTER (WHERE rating = 2) AS total_2_rating, COUNT(*) FILTER (WHERE rating = 1) AS total_1_rating FROM tbl_feedback WHERE tutor_id = $1`,
      [tutor_id],
    );
...
exports.respondToFeedback = async (req, res) => {
  try {
    ...
    const result = await con.query(
      `UPDATE tbl_feedback SET response = $1 WHERE feedback_id = $2`,
