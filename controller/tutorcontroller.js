...
exports.addDemoVideo = async (req, res) => {
 ...
 const video_file_key = await uploadToS3(req.files.video_file[0], 'tutors/demo_videos');

    const query = `
      INSERT INTO tbl_demo_videos
      (tutor_id, video_title, video_description, video_file, short_bio, 
       teaching_style, status, plan_type, royalty_percentage, price, 
       bank_upi_data)
